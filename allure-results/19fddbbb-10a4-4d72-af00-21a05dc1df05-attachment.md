...
Call log:
  - waiting for getByText('Dashboard').first() to be visible
...
```ts
  ...
  50 |     }
> 51 |     await this.dashboardMenuItem.waitFor({ state: 'visible' });
     |                                  ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  52 |     await this.leftNav.waitFor({ state: 'visible' });
  53 |     const link = this.leftNav.getByRole('link', { name, exact: true });
  54 |     await link.waitFor({ state: 'visible' });
  55 |     await link.click();
  56 |   }
  57 | 
  58 |   async clickQuickLaunch(name: string) {
  59 |     const button = this.page.getByRole('button', { name, exact: true });
  60 |     await button.waitFor({ state: 'visible' });
